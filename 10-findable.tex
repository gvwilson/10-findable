...
principles are a framework for doing this: they help researchers to define the full scope of their
users and needs, and to leverage the structural elements of the software that
creates, stores, and accesses information--whether data, research findings, or academic communication stored locally or shared on the internet--to improve findability.
...
audiences in a professional research or teaching capacity.
...
To solve the information retrieval problem, researchers must therefore think broadly
...
Spending time thinking about who your users are and how they currently interact with you and your work product is the foundation of all ten tips.
...
Reaching out to a small sampling of your users and asking for findability feedback may help you see any gaps in organizational alignment between you and your uers.
...
Returning to the faculty website from the previous tip as just one example of varied needs:
...
\item
  The librarian will want bibliographic information for your publications in machine-readable form
  (e.g., BibTeX, MARC, or MODS), either for individual items or in bulk in order to add them to an institutional repository.


\item
  Colleagues will want all of the above, plus pointers to the software and data
  used in order to reproduce particular results and.
...
adopt the same tools, so it is worth exploring best practices and innovations in your discipline and topic for 
annotating information as well as creating, manipulating, or storing it. For
example:
...
On a practical level, templates for file creation, data collection and lab notebooks ensure researchers maximize options for textual structure and ensure consistency among collaborators.
...
While much else has been written on web usability \cite{Covert2015, NNG2020}, library science focuses on information-seeking behavior.  
For example, we know that users scan but don't read: they click on the first
close thing they see and give up very, very quickly \cite{Bates2002}. Your markers and directions
...
create.  While software determines specific options, many digital tools allow users to add arbitrary tags to items.
File properties and GitHub are two examples of software-neutral platforms that allow for tags.
Additionally,
almost all search tools can utilize tags to narrow a query's scope. This means that you
...
Information to be tagged can range from informal personal use to data sets submitted with publications or included in repositories.  
The benefit of tagging comes from using the appropriate terms in all of those situations, not just when a journal submission requires it.

For work product that serves just you and your future self, you can create your own taxonomy of subject keywords, which is
called a \emph{folksonomy}. Folksonomies are what you see with tags on Flickr: 
early content creators assign terms as they see fit, and later
contributors can use those or add their own. If you take this route, it's worth
reviewing new tags regularly to look for synonyms, misspellings, differences in
capitalization, singular/plural discrepancies, and other inconsistencies. 

Exactly which terms you use as tags for personal consumption may not matter much, but work that is shared out to colleagues and the research community often require or encourage tags that conform to certain standards and/or use particular terms \cite{FAIR2020}.
Common term lists to standardize terms used to tag content--both data and textual information--include taxonomies, ontologies and thesauri.
Thesauri and taxonomies
generally have built-in subject hierarchies that can help you create navigational
structure, while ontologies map relationships between ideas, but all three are \emph{controlled vocabularies}.  Controlled means that they have a
defined list of terms created and maintained by experts, rather than the crowdsourced approach of a folksonomy. There may or may not be
relationships built between terms in a controlled vocabulary, such as equivalencies (``CA'' for
...
article databases, data repositories, and library catalogs that you and your users might already be
familiar with, which will again aid search and navigation.  Well-known examples in the United States include the National Cancer Institute (NCI) Thesaurus /cite{NCI2020} and the Medical Subject Headings (MeSH) \cite{ASI2020}.
