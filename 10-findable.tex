...
have. By doing this, you identify what research items can go where, and update them if
necessary before materials are published in a peer-reviewed journal or cited in a 
grant application. 

Consider this typical research workflow (Figure 1). A researcher uses a particular set of 
data sources (Fig 1A). These sources can take the form of model organisms or molecular systems which 
can be used to learn about living organisms or disease. Data sources might also include publicly 
available next-generation sequencing data or locally curated collections of microscopy images.
In the lab, data sources are treated with specialized protocols and tools (Fig 1B) which help visualize, 
...
Recent advancements in best-practices for making research products more 'FAIR' has 
led to the creation of a vast array of discipline specific, and general repositories, for 
researchers to deposit products created from each of the steps in the research workflow.
See PLoS's curated list for data repositories (https://journals.plos.org/plosone/s/recommended-repositories).
Many repositories now create Digital Object Identifers (DOIs), or other permanent identifiers
for their submissions, making them citable and easily linked to contributing researchers via Open 
Researcher and Contributor IDs (ORCIDs) (Fig 1G). Many repositories follow the standards
...
Let's think about our faculty member from the previous tip. She wants her lab's data to be stored
so she can cite it in multiple publications, both her original reserch and course materials. Her 
collaborators also need access to some, but not all of her lab's protocols. She might use the
following repositories which she found through PLoS's database of repositories, or recommended tools.

\begin{itemize}

\item
  She stores her data in a Dryad data repository so she can cite it in
  multiple publications with a DOI. She can update the data archive, as new
  pertinant data is collected and a new version of the data set is collected. (ref)
 
\item
  Her lab might mantains protocols in Protocols.io (https://doi.org/10.1371/journal.pbio.1002538)
  so that her collaborators and future students can easily find her established
  protocols using a DOI, while keeping incomplete protocol drafts private.

\item
  She posts finalized drafts of her student's research manuscripts on BioRxiv as they undergo
  peer review at a journal so her colleauges can download the draft easily and 
  discuss in an upcoming Journal Club.

\item
  Code used to analyze the data is deposited in Zenodo which she can cite and update
  as code gets optimized and new versions are created.
  
\item
  Each of these repositories create Digital Object Identifiers (DOIs) \cite{DOI2020} which she
  links to her personal ORCID \cite{ORCID2020} identifier. This way all her reserach products can
  be found easily by her students and her tenure committee with one link.

\end{itemize}

By planning ahead to figure out what records need to be preserved it can help ensure
your research is accessible and reusable. Nearly all major reserach products can be citable and 
interlinked, making them much easier to find for everyone that you might work with. 
Once these items are easily citable, we can explore strategies one can use to make it 
easier to find items within an article, or on a specific webpage. Remember that your 
future lab members are also your users: they will need to find data you collected 
long after you forgot where it went, so anything you do for others will likely pay 
off for yourself eventually\cite{Briney2015}.
...
Users might want content organized chronologically or topically.
