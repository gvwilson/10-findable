...
%DIF DEL /Users/gvwilson/writing/findable/first-submission.tex   Sat May 16 06:53:14 2020
%DIF ADD /Users/gvwilson/writing/findable/10-findable.tex        Sat May 16 06:45:21 2020
\include{settings}
...
principles are a framework for doing this: they help researchers \DIFdelbegin \DIFdel{to define their usersand needs , and to leverage }\DIFdelend \DIFaddbegin \DIFadd{improve
findability by defining the full scope of their users' needs and leveraging }\DIFaddend the
structural elements of the software that creates, stores, and accesses \DIFdelbegin \DIFdel{information to improve findability}\DIFdelend \DIFaddbegin \DIFadd{data,
...
this \cite{Rosenfeld2015,Hedden2016}. The problem is not just finding \DIFdelbegin \DIFdel{a
particular
bit of }\DIFdelend \DIFaddbegin \DIFadd{particular
}\DIFaddend information when needed: information may be stored in many formats, exist in
multiple versions, and need to be shared with varied audiences \DIFaddbegin \DIFadd{for both research
and teaching}\DIFaddend .
...
\DIFaddbegin \DIFadd{broadly }\DIFaddend about who needs that information and the context within which it is
...
making your actual user base both larger and more diverse.  \DIFaddbegin \DIFadd{Thinking about who
...
\DIFaddbegin \DIFadd{Reaching out to just a handful of users to ask for findability feedback will
help you see any gaps in organizational alignment.
}

\DIFaddend \rulemajor{2. Figure out what ``done'' looks like.}
...
Returning to the faculty website from the previous tip \DIFaddbegin \DIFadd{as just one example of
varied needs}\DIFaddend :

\begin{itemize}

\item
  \DIFaddbegin \DIFadd{Tenure committee members will want human-readable descriptions of related sets
  of papers along with links to presentations or posters that discuss them.
 }

\item
  \DIFaddend The librarian will want bibliographic information \DIFaddbegin \DIFadd{for your publications }\DIFaddend in
  machine-readable form (e.g., BibTeX, MARC, or MODS), either for individual
  items or in bulk \DIFaddbegin \DIFadd{for addition to an institutional repository}\DIFaddend .

\item
  \DIFdelbegin \DIFdel{Tenure committee members will want human-readable descriptions of related sets
  of papers along with links to presentations or posters that discuss them.
}%DIFDELCMD < 
...
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdelend Colleagues will want all of the above, plus pointers to the software and data
  used \DIFdelbegin \DIFdel{to produce }\DIFdelend \DIFaddbegin \DIFadd{in order to reproduce }\DIFaddend particular results.
...
adopt the same tools, so it is worth \DIFdelbegin \DIFdel{finding out how your domain's favorites can
}\DIFdelend \DIFaddbegin \DIFadd{exploring how your peers }\DIFaddend annotate
...
\DIFaddbegin \DIFadd{On a practical level, templates for file creation, data collection, and lab
notebooks makes it easier to be consistent and to spot inconsistencies.
}

\DIFaddend \rulemajor{4. Add metadata.}
...
\DIFdelbegin \DIFdel{Always remember, though, }\DIFdelend \DIFaddbegin \DIFadd{While much has been written about web usability in general
\mbox{%DIFAUXCMD
\cite{Covert2014,NNG2020}}\hspace{0pt}%DIFAUXCMD
, library science focuses on information-seeking
behavior.  For example, we know }\DIFaddend that users scan but don't read: they click on
the first close thing they see and give up very, very quickly
\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Bates2002}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Your markers and directions should therefore be as consistent
...
create.  Almost all digital tools allow users to add arbitrary tags to items\DIFdelbegin \DIFdel{,
and }\DIFdelend \DIFaddbegin \DIFadd{:
file properties on Windows and labels on GitHub issues are just two examples.
Additionally, }\DIFaddend almost all search tools \DIFdelbegin \DIFdel{use them to narrow their focus}\DIFdelend \DIFaddbegin \DIFadd{leverage tags to narrow a query's
scope}\DIFaddend . This means that you can now file a single thing in multiple
...
\DIFdelbegin \DIFdel{Exactly which terms you }\DIFdelend \DIFaddbegin \DIFadd{What should you tag?  The answer is ``everything'' from informal personal notes
...
\DIFadd{If you are certain something is for purely personal use, you can create your own
taxonomy of subject keywords, which is called a }\emph{\DIFadd{folksonomy}}\DIFadd{. Folksonomies
...
\DIFadd{What terms you }\DIFaddend use as tags \DIFaddbegin \DIFadd{for personal consumption }\DIFaddend may not matter much\DIFdelbegin \DIFdel{for your discipline
beyond a need for consistency, but
if your discipline has an established
...
conform to certain standards \mbox{%DIFAUXCMD
\cite{FAIR2020}}\hspace{0pt}%DIFAUXCMD
.  These terms typically come from
taxonomies, thesauri, and ontologies: taxonomies and thesauri generally }\DIFaddend have
built-in subject hierarchies that can help you create navigational structure\DIFdelbegin \DIFdel{. Examples include the Astronomy Thesaurus,
the Getty Art \&
...
Geosciences, and the Medical Subject Headings (MeSH) \mbox{%DIFAUXCMD
\cite{ASI2020}}\hspace{0pt}%DIFAUXCMD
. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{These types of term lists }\DIFdelend \DIFaddbegin \DIFadd{,
while ontologies map relationships between ideas. Crucially, all three }\DIFaddend are
\emph{controlled vocabularies}: they \DIFdelbegin \DIFdel{have }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend a defined list of terms created and
maintained by experts \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{rather than being crowdsourced like a folksonomy.
}

\DIFaddend There may or may not be relationships built between terms \DIFaddbegin \DIFadd{in a controlled
vocabulary}\DIFaddend , such as equivalencies (``CA'' for ``California''), broader/narrower
terms (United States/California), and/or replacement (weed \emph{use}
marijuana). Established subject terms will match article databases\DIFaddbegin \DIFadd{, data
repositories, }\DIFaddend and library catalogs that you and your users might already be
familiar with, which will again aid search and navigation.  \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Alternatively, you can create your own taxonomy of subject keywords, which is
called a }\emph{\DIFdel{folksonomy}}%DIFAUXCMD
\DIFdel{. Folksonomies are what you see with tags on Flickr
...
reviewing new tags regularly to look for synonyms, misspellings, differences in capitalization, singular/plural discrepancies, and other inconsistencies.  Doing
this will also reveal new tags that you may wish to adopt, which in turn gives
you insight into how people are viewing your information}\DIFdelend \DIFaddbegin \DIFadd{Well-known examples
in the United States include the National Cancer Institute (NCI) Thesaurus
\mbox{%DIFAUXCMD
\cite{NCI2020} }\hspace{0pt}%DIFAUXCMD
and the Medical Subject Headings (MeSH) \mbox{%DIFAUXCMD
\cite{ASI2020}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
